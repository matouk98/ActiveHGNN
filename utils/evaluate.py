...
def eval_metrics(pred, lbl):
    acc = torch.sum(pred == lbl).float() / lbl.shape[0]
    f1_macro = f1_score(lbl.cpu(), pred.cpu(), average='macro')
    f1_micro = f1_score(lbl.cpu(), pred.cpu(), average='micro')
    return acc.item(), f1_macro.item(), f1_micro.item()
